  ...
  try {
    const stdout = await new Promise<string>((resolve, reject) => {
      ...
      const timeout = setTimeout(() => {
        ...
        child.kill('SIGTERM')
        reject(
          new Error('command timed out; interactive prompts are not supported')
        )
      }, COMMAND_TIMEOUT_MS)
    ...
    if (!parsedEnvValues.GOOGLE_CREDENTIALS_JSON) {
      throw new Error(
        'command output did not contain GOOGLE_CREDENTIALS_JSON variable'
      )
    }
...
async function readSpreadsheet(
  ...
  try {
    ...
    const message = error instanceof Error ? error.message : String(error)
    throw new Error(
      `Failed to read spreadsheet with profile "${profile.name}": ${message}`
    )
  }
}
...
async function readWithFallback(
  ...
  let res: {
    data: Awaited<ReturnType<typeof readSpreadsheet>>
    profile: Profile
  } | null = null
